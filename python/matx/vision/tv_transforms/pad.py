...
class PadImpl(BatchBaseClass):
    def __init__(self,
        ...
        if len(padding)==1:
            ...
            _assert(False, "Padding must be an int or a 1, 2, or 4 element tuple, not a {} element tuple".format(len(padding)))
    ...
    def _process(self, imgs: List[matx.NDArray]) -> List[matx.NDArray]:
        return self.op(imgs, [self.padding[1]], [self.padding[3]], [self.padding[0]], [self.padding[2]], sync=self.sync)
