...
class RandomAffine(BaseInterfaceClass):
    ...
    def __call__(self, device: Any, device_str: str, sync: int) -> Any:
        return RandomAffineImpl(device, device_str, self.degrees, self.translate, self.scale, self.shear, self.interp, self.fill, sync)
...
class RandomAffineImpl(BatchBaseClass):
    ...
    def _create_matrix_with_param(self, center:Tuple[float, float], angle:float, translate:Tuple[float, float], scale:float, shear:Tuple[float, float])->List[List[float]]:
...
class RandomPerspective(BaseInterfaceClass):
    ...
    def __call__(self, device: Any, device_str: str, sync: int) -> Any:
        return RandomPerspectiveImpl(device, device_str, self.distortion_scale, self.interp, self.fill,  self.p, sync)
...
class RandomPerspectiveImpl(BatchRandomBaseClass):
    def __init__(self, 
        ...
        self.distortion_scale: float = distortion_scale
        self.fill: Tuple[float, float, float] = fill
        self.op: RotateOp = WarpPerspectiveOp(device, pad_values=self.fill, interp=self.interpolation)
        self.sync:int = sync
    ...
    def get_points(self, image_shape:List[int])->List[List[Tuple[float, float]]]:
        ...
        startpoints:List[Tuple[float, float]] = [(0.0, 0.0), (float(w - 1), 0.0), (float(w - 1), float(h - 1)), (0.0, float(h - 1))]
