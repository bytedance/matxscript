...
class ColorJitter(BaseInterfaceClass):
    ...
    def __call__(self, device: Any, device_str: str, sync: int) -> Any:
        return ColorJitterImpl(device, device_str, self.brightness, self.contrast, self.saturation, self.hue, sync)
...
class ColorJitterImpl(BatchBaseClass):
    def __init__(self,
        ...
        self.hue:List[float] = self._check_input(hue, "hue", center=0, bound=(-0.5, 0.5), clip_first_on_zero=False)
    ...
    def _check_input(self, 
        ...
        _assert(len(value)>0 and len(value) <= 2, "{} should be a single number or a list with length 2.".format(name))
        if len(value)==1:
            _assert(value[0] >=0, "If {} is a single number, it must be non negative.".format(name))
            value = [center - value[0], center + value[0]]
            if clip_first_on_zero:
                value[0] = max(value[0], 0)
        _assert(bound[0] <= value[0] <= value[1] <= bound[1], "{} values should be between {}".format(name, bound))
        if value[0] == value[1] == center:
            value = []
        return value

    def _brightness(self, imgs:List[matx.NDArray], factors:List[float], sync:int)->List[matx.NDArray]:
        size:int = len(imgs)
        shifts:List[int] = [0] * size
        return self.rgb_adjust_op(imgs, factors, shifts, sync=sync)

    def _contrast(self, imgs:List[matx.NDArray], factors:List[float], sync:int)->List[matx.NDArray]:
    ...
    def _saturation(self, imgs:List[matx.NDArray], factors:List[float], sync:int)->List[matx.NDArray]:
        gray:List[matx.NDArray] = self.gray_op(imgs)
    ...
    def get_params(self)->Dict[str, Tuple[int, List[float]]]:
        ...
        b:List[float] = [] if len(self.brightness)==0 else [random.uniform(self.brightness[0], self.brightness[1])]
        c:List[float] = [] if len(self.contrast)==0 else [random.uniform(self.contrast[0], self.contrast[1])]
        s:List[float] = [] if len(self.saturation)==0 else [random.uniform(self.saturation[0], self.saturation[1])]
    ...
    def _process(self, imgs: List[matx.NDArray])->List[matx.NDArray]:
        ...
        for i, r in enumerate(rounds_info):
            ...
            for fn_name in r:
                ...
                result_imgs:List[matx.NDArray] = self._call_func(fn_name,target_imgs, factors, sync)
                imgs = self._put_back_converted_images(imgs, img_idx, result_imgs)
        return imgs

    def _call_func(self, func_name:str, target:List[matx.NDArray], factor:List[float], sync:int)->List[matx.NDArray]:
