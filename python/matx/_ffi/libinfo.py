...
def find_lib_path(name=None, search_path=None, optional=False):
    ...
    """
    use_runtime = os.environ.get("MATX_USE_RUNTIME_LIB", False)
    ...
    if os.environ.get('MATX_LIBRARY_PATH', None):
        dll_path.append(os.environ['MATX_LIBRARY_PATH'])
    ...
    if name is not None:
        ...
        if sys.platform.startswith('win32'):
            lib_dll_path = [os.path.join(p, 'libmatx_runtime.dll') for p in dll_path] + \
                           [os.path.join(p, 'matx_runtime.dll') for p in dll_path]
            runtime_dll_path = [os.path.join(p, 'libmatx.dll') for p in dll_path] + \
                               [os.path.join(p, 'matx.dll') for p in dll_path]
        elif sys.platform.startswith('darwin'):
            lib_dll_path = [os.path.join(p, 'libmatx.dylib') for p in dll_path]
            runtime_dll_path = [os.path.join(p, 'libmatx_runtime.dylib') for p in dll_path]
        else:
            lib_dll_path = [os.path.join(p, 'libmatx.so') for p in dll_path]
            runtime_dll_path = [os.path.join(p, 'libmatx_runtime.so') for p in dll_path]
...
def find_include_path(name=None, search_path=None, optional=False):
    ...
    if os.environ.get('MATX_INCLUDE_PATH', None):
        header_path.append(os.environ['MATX_INCLUDE_PATH'])
    ...
    if name is not None:
        if isinstance(name, list):
            matx_include_path = []
            for n in name:
                matx_include_path += [os.path.join(p, n) for p in header_path]
        else:
            matx_include_path = [os.path.join(p, name) for p in header_path]
        dlpack_include_path = []
        include_found = [p for p in matx_include_path if os.path.exists(p) and os.path.isdir(p)]
    else:
        matx_include_path = [os.path.join(p, 'include') for p in header_path]
    ...
    if not include_found:
        message = ('Cannot find the files.\n' +
                   'List of candidates:\n' +
                   str('\n'.join(matx_include_path + dlpack_include_path)))
        if not optional:
            raise RuntimeError(message)
        return None
