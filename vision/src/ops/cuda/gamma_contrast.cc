...
class VisionGammaContrastOpGPU : public VisionBaseImageOpGPU<cuda_op::GammaContrastVarShape> {
 public:
  VisionGammaContrastOpGPU(const Any& session_info)
      : VisionBaseImageOpGPU<cuda_op::GammaContrastVarShape>(session_info){};
  RTValue process(const List& images, const List& gamma, bool per_channel, int sync);
};

RTValue VisionGammaContrastOpGPU::process(const List& arg_images,
                                          const List& gamma_in,
                                          bool per_channel,
                                          int sync) {
  ...
  for (const RTValue& nd_elem : images) {
    ...
    if (per_channel) {
      for (int ch = 0; ch < channel; ch++) {
        gamma[i * channel + ch] = gamma_in[i * channel + ch].As<float>();
      }
    } else {
      gamma[i] = gamma_in[i].As<float>();
    }
...
MATX_REGISTER_NATIVE_OBJECT(VisionGammaContrastOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 1) << "[VisionGammaContrastOpGPU] Expect 1 arguments but get "
                                 << args.size();
      return std::make_shared<VisionGammaContrastOpGPU>(args[0]);
    })
    .RegisterFunction("process", [](void* self, PyArgs args) -> RTValue {
      MXCHECK_EQ(args.size(), 4)
          << "[VisionGammaContrastOpGPU][func: process] Expect 4 arguments but get " << args.size();
      return reinterpret_cast<VisionGammaContrastOpGPU*>(self)->process(
