...
class VisionHistEqualizeOpGPU : public VisionBaseImageOpGPU<cuda_op::HistEqualizeVarShape> {
 public:
  VisionHistEqualizeOpGPU(const Any& session_info)
      : VisionBaseImageOpGPU<cuda_op::HistEqualizeVarShape>(session_info){};
  RTValue process(const List& images, int sync);
};

RTValue VisionHistEqualizeOpGPU::process(const List& arg_images, int sync) {
...
MATX_REGISTER_NATIVE_OBJECT(VisionHistEqualizeOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 1) << "[VisionHistEqualizeOpGPU] Expect 1 arguments but get "
                                 << args.size();
      return std::make_shared<VisionHistEqualizeOpGPU>(args[0]);
    })
    .RegisterFunction("process", [](void* self, PyArgs args) -> RTValue {
      MXCHECK_EQ(args.size(), 2)
          << "[VisionHistEqualizeOpGPU][func: process] Expect 2 arguments but get " << args.size();
      return reinterpret_cast<VisionHistEqualizeOpGPU*>(self)->process(
