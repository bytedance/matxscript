...
class VisionChannelReorderOpGPU : public VisionBaseImageOpGPU<cuda_op::ChannelReorderVarShape> {
 public:
  VisionChannelReorderOpGPU(const Any& session_info)
      : VisionBaseImageOpGPU<cuda_op::ChannelReorderVarShape>(session_info){};
  RTValue process(const List& images, const List& orders, int out_channel, int sync);
};

RTValue VisionChannelReorderOpGPU::process(const List& arg_images,
                                           const List& orders_in,
                                           int out_channel,
                                           int sync) {
  ...
  size_t op_buffer_size = op_->calBufferSize(batch_size, out_channel);
  ...
  List res;
  int orders[out_channel * batch_size];
  int i = 0;
  for (const RTValue& nd_elem : images) {
    ...
    MXCHECK(src_shape.size() >= 2)
        << "Invalid input. The dim of image shape should be equal or larger than 2.";
    if (src_shape.size() == 2)
    ...
    if (i == 0) {
      ...
      if (channel != src_shape[2]) {
        MXCHECK(false) << "Invalid input. The input images should have the same number of channels";
      }
    ...
    std::vector<int64_t> out_shape = {src_shape[0], src_shape[1], out_channel};
    ...
    auto order_view = orders_in[i].AsObjectView<List>();
    const List& cur_orders = order_view.data();
    MXCHECK_EQ(cur_orders.size(), out_channel)
        << "Invalid input!"
        << "The order size of the new channels should be equal to out channel.";
    for (int ch = 0; ch < out_channel; ch++) {
      int ch_idx = cur_orders[ch].As<int>();
      MXCHECK(ch_idx < channel)
          << "Invalid input. The channel index should be less than the input channel size";
      orders[out_channel * i + ch] = ch_idx;
    }
...
MATX_REGISTER_NATIVE_OBJECT(VisionChannelReorderOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 1) << "[VisionChannelReorderOpGPU] Expect 1 arguments but get "
                                 << args.size();
      return std::make_shared<VisionChannelReorderOpGPU>(args[0]);
    })
    .RegisterFunction("process", [](void* self, PyArgs args) -> RTValue {
      MXCHECK_EQ(args.size(), 4)
          << "[VisionChannelReorderOpGPU][func: process] Expect 4 arguments but get "
          << args.size();
      return reinterpret_cast<VisionChannelReorderOpGPU*>(self)->process(
