...
class VisionConv2dOpGPU : public VisionBaseImageOpGPU<cuda_op::Conv2DVarShape> {
 public:
  VisionConv2dOpGPU(const Any& session_info)
      : VisionBaseImageOpGPU<cuda_op::Conv2DVarShape>(session_info){};
...
RTValue VisionConv2dOpGPU::process(const List& arg_images,
                                   const List& kernel_in,
                                   const List& ksize_in,
  ...
  for (int i = 0; i < batch_size; i++) {
    auto ksize_view = ksize_in[i].AsObjectView<List>();
    const List& cur_ksize = ksize_view.data();
    int ksize_x = cur_ksize[0].As<int>();
    int ksize_y = cur_ksize[1].As<int>();
    cv_ksize[i] = cv::Size(ksize_x, ksize_y);
    kernel_total_size += ksize_x * ksize_y;
  }
  ...
  float kernel[kernel_total_size];
  ...
  for (const RTValue& nd_elem : images) {
    ...
    auto kernel_view = kernel_in[i].AsObjectView<List>();
    const List& cur_kernel = kernel_view.data();
    for (int k = 0; k < cv_ksize[i].width * cv_ksize[i].height; k++) {
      kernel[kidx] = cur_kernel[k].As<float>();
      kidx += 1;
    }
...
MATX_REGISTER_NATIVE_OBJECT(VisionConv2dOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 1) << "[VisionConv2dOpGPU] Expect 1 arguments but get "
                                 << args.size();
      return std::make_shared<VisionConv2dOpGPU>(args[0]);
    })
    .RegisterFunction("process", [](void* self, PyArgs args) -> RTValue {
      MXCHECK_EQ(args.size(), 6) << "[VisionConv2dOpGPU][func: process] Expect 6 arguments but get "
                                 << args.size();
      return reinterpret_cast<VisionConv2dOpGPU*>(self)->process(
