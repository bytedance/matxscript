...
class VisionGaussianBlurOpGPU : public VisionBaseImageOpGPU<cuda_op::GaussianVarShape> {
 public:
  VisionGaussianBlurOpGPU(const Any& session_info)
      : VisionBaseImageOpGPU<cuda_op::GaussianVarShape>(session_info){};
...
RTValue VisionGaussianBlurOpGPU::process(const List& arg_images,
                                         const List& ksize_in,
                                         const List& sigma_in,
                                         const unicode_view& borderType,
                                         int sync) {
  ...
  List res;
  double sigma_x[batch_size];
  double sigma_y[batch_size];
  ...
  for (const RTValue& nd_elem : images) {
    ...
    auto sigma_view = sigma_in[i].AsObjectView<Tuple>();
    const Tuple& cur_sigma = sigma_view.data();
    sigma_x[i] = cur_sigma[0].As<double>();
    sigma_y[i] = cur_sigma[1].As<double>();
...
MATX_REGISTER_NATIVE_OBJECT(VisionGaussianBlurOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 1) << "[VisionGaussianBlurOpGPU] Expect 1 arguments but get "
                                 << args.size();
      return std::make_shared<VisionGaussianBlurOpGPU>(args[0]);
    })
    .RegisterFunction("process", [](void* self, PyArgs args) -> RTValue {
      MXCHECK_EQ(args.size(), 5)
          << "[VisionGaussianBlurOpGPU][func: process] Expect 5 arguments but get " << args.size();
      return reinterpret_cast<VisionGaussianBlurOpGPU*>(self)->process(
