...
class VisionSolarizeOpGPU : public VisionBaseImageOpGPU<cuda_op::SolarizeVarShape> {
 public:
  VisionSolarizeOpGPU(const Any& session_info)
      : VisionBaseImageOpGPU<cuda_op::SolarizeVarShape>(session_info){};
  RTValue process(const List& images, const List& threshold, int sync);
};

RTValue VisionSolarizeOpGPU::process(const List& arg_images, const List& threshold, int sync) {
  ...
  for (const RTValue& nd_elem : images) {
    ...
    threshold_[i] = threshold[i].As<float>();
...
MATX_REGISTER_NATIVE_OBJECT(VisionSolarizeOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 1) << "[VisionSolarizeOpGPU] Expect 1 arguments but get "
                                 << args.size();
      return std::make_shared<VisionSolarizeOpGPU>(args[0]);
    })
    .RegisterFunction("process",
                      [](void* self, PyArgs args) -> RTValue {
                        MXCHECK_EQ(args.size(), 3)
                            << "[VisionSolarizeOpGPU][func: process] Expect 3 arguments but get "
                            << args.size();
                        return reinterpret_cast<VisionSolarizeOpGPU*>(self)->process(
