...
class VisionGaussNoiseOpGPU : public VisionBaseOpGPU {
 public:
  VisionGaussNoiseOpGPU(const Any& session_info, int batch_size);
  RTValue process(
      const List& images, const List& mu, const List& sigma, bool per_channel, int sync);

 private:
  std::shared_ptr<cuda_op::GaussNoiseVarShape> op_;
  cuda_op::DataShape max_input_shape_, max_output_shape_;
};

VisionGaussNoiseOpGPU::VisionGaussNoiseOpGPU(const Any& session_info, int batch_size)
  ...
  op_ = std::make_shared<cuda_op::GaussNoiseVarShape>(max_input_shape_, max_output_shape_);
  op_->setUpRand(time(NULL));
}

RTValue VisionGaussNoiseOpGPU::process(
    const List& arg_images, const List& mu_in, const List& sigma_in, bool per_channel, int sync) {
  ...
  for (const RTValue& nd_elem : images) {
    ...
    mu[i] = mu_in[i].As<float>();
    sigma[i] = sigma_in[i].As<float>();
...
MATX_REGISTER_NATIVE_OBJECT(VisionGaussNoiseOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 2) << "[VisionGaussNoiseOpGPU] Expect 2 arguments but get "
                                 << args.size();
      return std::make_shared<VisionGaussNoiseOpGPU>(args[1], args[0].As<int>());
    })
    .RegisterFunction("process",
                      [](void* self, PyArgs args) -> RTValue {
                        MXCHECK_EQ(args.size(), 5)
                            << "[VisionGaussNoiseOpGPU][func: process] Expect 5 arguments but get "
                            << args.size();
                        return reinterpret_cast<VisionGaussNoiseOpGPU*>(self)->process(
