...
class VisionLaplacianBlurOpGPU : public VisionBaseImageOpGPU<cuda_op::LaplacianVarShape> {
 public:
  VisionLaplacianBlurOpGPU(const Any& session_info)
      : VisionBaseImageOpGPU<cuda_op::LaplacianVarShape>(session_info){};
...
RTValue VisionLaplacianBlurOpGPU::process(const List& arg_images,
                                          const List& ksize_in,
                                          const List& scale_in,
                                          const unicode_view& borderType,
                                          int sync) {
  ...
  List res;
  double scale[batch_size];
  int ksize[batch_size];
  ...
  for (const RTValue& nd_elem : images) {
    ...
    scale[i] = scale_in[i].As<double>();
    ksize[i] = ksize_in[i].As<int>();
...
MATX_REGISTER_NATIVE_OBJECT(VisionLaplacianBlurOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 1) << "[VisionLaplacianBlurOpGPU] Expect 1 arguments but get "
                                 << args.size();
      return std::make_shared<VisionLaplacianBlurOpGPU>(args[0]);
    })
    .RegisterFunction("process", [](void* self, PyArgs args) -> RTValue {
      MXCHECK_EQ(args.size(), 5)
          << "[VisionLaplacianBlurOpGPU][func: process] Expect 5 arguments but get " << args.size();
      return reinterpret_cast<VisionLaplacianBlurOpGPU*>(self)->process(
