...
class VisionMedianBlurOpGPU : public VisionBaseImageOpGPU<cuda_op::MedianBlurVarShape> {
 public:
  VisionMedianBlurOpGPU(const Any& session_info)
      : VisionBaseImageOpGPU<cuda_op::MedianBlurVarShape>(session_info){};
  RTValue process(const List& images, const List& ksizes, int sync);
};

RTValue VisionMedianBlurOpGPU::process(const List& arg_images, const List& ksize_in, int sync) {
...
MATX_REGISTER_NATIVE_OBJECT(VisionMedianBlurOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 1) << "[VisionMedianBlurOpGPU] Expect 1 arguments but get "
                                 << args.size();
      return std::make_shared<VisionMedianBlurOpGPU>(args[0]);
    })
    .RegisterFunction("process",
                      [](void* self, PyArgs args) -> RTValue {
                        MXCHECK_EQ(args.size(), 3)
                            << "[VisionMedianBlurOpGPU][func: process] Expect 3 arguments but get "
                            << args.size();
                        return reinterpret_cast<VisionMedianBlurOpGPU*>(self)->process(
