...
class VisionLinearAdjustOpGPU : public VisionBaseImageOpGPU<cuda_op::AdjustColorLinearVarShape> {
 public:
  VisionLinearAdjustOpGPU(const Any& session_info)
      : VisionBaseImageOpGPU<cuda_op::AdjustColorLinearVarShape>(session_info){};
  RTValue process(
      const List& images, const List& factors, const List& shifts, bool per_channel, int sync);
};

RTValue VisionLinearAdjustOpGPU::process(
    const List& arg_images, const List& factors, const List& shifts, bool per_channel, int sync) {
  ...
  List res;
  float factors_[batch_size * 4], shifts_[batch_size * 4];
  ...
  for (const RTValue& nd_elem : images) {
    ...
    if (per_channel) {
      for (int ch = 0; ch < channel; ch++) {
        factors_[i * channel + ch] = factors[i * channel + ch].As<float>();
        shifts_[i * channel + ch] = shifts[i * channel + ch].As<float>();
      }
    } else {
      factors_[i] = factors[i].As<float>();
      shifts_[i] = shifts[i].As<float>();
    }
...
MATX_REGISTER_NATIVE_OBJECT(VisionLinearAdjustOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 1) << "[VisionLinearAdjustOpGPU] Expect 1 arguments but get "
                                 << args.size();
      return std::make_shared<VisionLinearAdjustOpGPU>(args[0]);
    })
    .RegisterFunction("process", [](void* self, PyArgs args) -> RTValue {
      MXCHECK_EQ(args.size(), 5)
          << "[VisionLinearAdjustOpGPU][func: process] Expect 5 arguments but get " << args.size();
      return reinterpret_cast<VisionLinearAdjustOpGPU*>(self)->process(
