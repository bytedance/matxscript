...
class VisionPosterizeOpGPU : public VisionBaseImageOpGPU<cuda_op::PosterizeVarShape> {
 public:
  VisionPosterizeOpGPU(const Any& session_info)
      : VisionBaseImageOpGPU<cuda_op::PosterizeVarShape>(session_info){};
  RTValue process(const List& images, const List& bits, int sync);
};

RTValue VisionPosterizeOpGPU::process(const List& arg_images, const List& bits, int sync) {
...
MATX_REGISTER_NATIVE_OBJECT(VisionPosterizeOpGPU)
    .SetConstructor([](PyArgs args) -> std::shared_ptr<void> {
      MXCHECK_EQ(args.size(), 1) << "[VisionPosterizeOpGPU] Expect 1 arguments but get "
                                 << args.size();
      return std::make_shared<VisionPosterizeOpGPU>(args[0]);
    })
    .RegisterFunction("process",
                      [](void* self, PyArgs args) -> RTValue {
                        MXCHECK_EQ(args.size(), 3)
                            << "[VisionPosterizeOpGPU][func: process] Expect 3 arguments but get "
                            << args.size();
                        return reinterpret_cast<VisionPosterizeOpGPU*>(self)->process(
