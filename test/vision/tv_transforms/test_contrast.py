...
from matx.vision.tv_transforms import Compose, RandomAutocontrast


script_path = os.path.dirname(os.path.abspath(os.path.expanduser(__file__)))


class TestRandomAutocontrast(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_random_auto_contrast(self):
        bytedvision_op = Compose(0, [RandomAutocontrast(p=1.0)])
        bytedvision_res = bytedvision_op([self.img_nd])[0].asnumpy()
        torchvision_op = transforms.RandomAutocontrast(p=1.0)
    ...
    def test_scripted_random_auto_contrast(self):
        op = matx.script(RandomAutocontrast)(p=1.0)
