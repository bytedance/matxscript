...
from matx.vision.tv_transforms import Compose, RandomHorizontalFlip, RandomVerticalFlip
...
class TestRandomHorizontalFlip(unittest.TestCase):
    ...
    def test_random_horizontal_flip_with_compose(self):
        bytedvision_op = Compose(0, [RandomHorizontalFlip(p=1.0)])
    ...
    def test_scripted_random_horizontal_flip_with_compose(self):
        op = matx.script(RandomHorizontalFlip)(p=1.0)
...
class TestRandomVerticalFlip(unittest.TestCase):
    ...
    def test_random_vertical_flip(self):
        bytedvision_op = Compose(0, [RandomVerticalFlip(p=1.0)])
        bytedvision_res = bytedvision_op([self.img_nd])[0].asnumpy()
        torchvision_op = transforms.RandomVerticalFlip(p=1.0)
        torchvision_res = np.array(torchvision_op(self.img_tensor))
        np.testing.assert_almost_equal(bytedvision_res, torchvision_res)

    def test_scripted_random_vertical_flip_with_compose(self):
        op = matx.script(RandomVerticalFlip)(p=1.0)
        composed_op = matx.script(Compose)(0, [op])
        bytedvision_res = composed_op([self.img_nd])[0].asnumpy()
        torchvision_op = transforms.RandomVerticalFlip(p=1.0)
        torchvision_res = np.array(torchvision_op(self.img_tensor))
