...
from matx.vision.tv_transforms import Compose, ConvertImageDtype


script_path = os.path.dirname(os.path.abspath(os.path.expanduser(__file__)))


class TestConvertImageDtype(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.img_tensor = torch.tensor(img).permute(2, 0, 1)
        return super().setUp()

    def test_convert_dtype(self):
        bytedvision_op = Compose(0, [ConvertImageDtype("float32", global_scale=1.0 / 255)])
        bytedvision_res = bytedvision_op([self.img_nd])[0].asnumpy()
        torchvision_op = transforms.ConvertImageDtype(torch.float)
    ...
    def test_scripted_convert_dtype(self):
        op = matx.script(ConvertImageDtype)("float32", global_scale=1.0 / 255)
