...
from matx.vision.tv_transforms import Compose, RandomEqualize


script_path = os.path.dirname(os.path.abspath(os.path.expanduser(__file__)))


class TestRandomEqualize(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_random_equalize(self):
        bytedvision_op = Compose(0, [RandomEqualize(p=1.0)])
        bytedvision_res = bytedvision_op([self.img_nd])[0].asnumpy()
        torchvision_op = transforms.RandomEqualize(p=1.0)
        torchvision_res = np.array(torchvision_op(self.img_tensor))
        diff = np.abs(bytedvision_res - torchvision_res)
        assert np.sum(diff >= 6) < 10

    def test_scripted_random_equalize(self):
        op = matx.script(RandomEqualize)(p=1.0)
