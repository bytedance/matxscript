...
from matx.vision.tv_transforms import Compose, RandomSolarize


script_path = os.path.dirname(os.path.abspath(os.path.expanduser(__file__)))


class TestRandomSolarize(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_random_solarize(self):
        thres = 100.0
        bytedvision_op = Compose(0, [RandomSolarize(thres, p=1.0)])
        bytedvision_res = bytedvision_op([self.img_nd])[0].asnumpy()
        torchvision_op = transforms.RandomSolarize(thres, p=1.0)
        torchvision_res = np.array(torchvision_op(self.img_tensor))
        np.testing.assert_almost_equal(bytedvision_res, torchvision_res)

    def test_scripted_random_solarize(self):
        thres = 100.0
        op = matx.script(RandomSolarize)(thres, p=1.0)
