...
from matx.vision.tv_transforms import Compose, RandomAdjustSharpness


script_path = os.path.dirname(os.path.abspath(os.path.expanduser(__file__)))


class TestRandomAdjustSharpness(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_random_shapeness(self):
        sharpness_factor = 1.2
        bytedvision_op = Compose(0, [RandomAdjustSharpness(sharpness_factor, p=1.0)])
        bytedvision_res = bytedvision_op([self.img_nd])[0].asnumpy()
        torchvision_op = transforms.RandomAdjustSharpness(sharpness_factor, p=1.0)
    ...
    def test_scripted_random_shapeness(self):
        sharpness_factor = 1.2
        op = matx.script(RandomAdjustSharpness)(sharpness_factor, p=1.0)
