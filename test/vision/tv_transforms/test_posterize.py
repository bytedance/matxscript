...
from matx.vision.tv_transforms import Compose, RandomPosterize


script_path = os.path.dirname(os.path.abspath(os.path.expanduser(__file__)))


class TestRandomPosterize(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_random_posterize(self):
        bits = 6
        bytedvision_op = Compose(0, [RandomPosterize(bits, p=1.0)])
        bytedvision_res = bytedvision_op([self.img_nd])[0].asnumpy()
        torchvision_op = transforms.RandomPosterize(bits, p=1.0)
        torchvision_res = np.array(torchvision_op(self.img_tensor))
        np.testing.assert_almost_equal(bytedvision_res, torchvision_res)

    def test_scripted_random_posterize(self):
        bits = 6
        op = matx.script(RandomPosterize)(bits, p=1.0)
