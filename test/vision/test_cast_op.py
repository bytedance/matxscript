...
class TestCastOp(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.images_nd = matx.array.from_numpy(images)
        self.dtype = "float32"
        self.device = matx.Device("cpu")

        return super().setUp()

    def test_cpu_cast_op(self):
        cast_op = CastOp(self.device)

        op_ret = cast_op(self.image_nd, self.dtype, self.alpha, self.beta)
        self._helper(op_ret, self.origin_res)

        op_ret_batch = cast_op(self.images_nd, self.dtype, self.alpha, self.beta)
        self._helper(op_ret_batch, self.origin_res_batch)

    def test_scripted_cpu_cast_op(self):
        script_cast_op = matx.script(CastOp)(self.device)

        script_ret = script_cast_op(self.image_nd, self.dtype, self.alpha, self.beta)
        self._helper(script_ret, self.origin_res)

        script_ret_batch = script_cast_op(self.images_nd, self.dtype, self.alpha, self.beta)
