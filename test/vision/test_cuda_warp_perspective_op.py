...
class TestCudaWarpPerspectiveOp(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _cuda_warp_perspective_cpu_input_sync(self, op):
        op_ret = op(self.image_nd_cpu, self.pts, [], byted_vision.SYNC_CPU)
        for nd in op_ret:
            self.assertEqual(nd.device(), "cpu")
        self._helper(op_ret)

    def test_cuda_warp_perspective_cpu_input_sync(self):
        op = byted_vision.WarpPerspectiveOp(self.device, pad_values=self.border_value)
        self._cuda_warp_perspective_cpu_input_sync(op)

    def test_cuda_warp_perspective_cpu_input_sync_scripted(self):
        op = matx.script(byted_vision.WarpPerspectiveOp)(self.device, pad_values=self.border_value)
        self._cuda_warp_perspective_cpu_input_sync(op)
