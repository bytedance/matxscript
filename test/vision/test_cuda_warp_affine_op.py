...
class TestCudaWarpAffineOp(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.image_nd_cpu = [matx.array.from_numpy(i) for i in self.images]
    ...
    def test_translate(self):
        ...
        affine_op = byted_vision.WarpAffineOp(
            self.device, interp=byted_vision.INTER_NEAREST)
        op_ret = affine_op(self.image_nd, matrix)
        self._helper(op_ret, origin_res)

        cpu_ret = affine_op(self.image_nd_cpu, matrix,
                            [], byted_vision.SYNC_CPU)
        for nd in cpu_ret:
            self.assertEqual(nd.device(), "cpu")
        self._helper(cpu_ret, origin_res)
        ...
        cpu_ret = affine_script_op(
            self.image_nd_cpu, matrix, [], byted_vision.SYNC_CPU)
        for nd in cpu_ret:
            self.assertEqual(nd.device(), "cpu")
        self._helper(cpu_ret, origin_res)
    ...
    def test_rotate(self):
        ...
        cpu_ret = affine_script_op(
            self.image_nd_cpu, matrix, dsize, byted_vision.SYNC_CPU)
        for nd in cpu_ret:
            self.assertEqual(nd.device(), "cpu")
        self._helper(cpu_ret, origin_res)
    ...
    def test_shear(self):
        ...
        affine_op = byted_vision.WarpAffineOp(
            self.device, interp=byted_vision.INTER_CUBIC)
        op_ret = affine_op(self.image_nd, matrix)
